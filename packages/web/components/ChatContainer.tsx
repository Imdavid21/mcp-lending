...
// The API is always on the same origin (/api/...).
// Override with NEXT_PUBLIC_CLIENT_URL to point at an external client server.
const API_BASE = process.env.NEXT_PUBLIC_CLIENT_URL ?? '';
...
export default function ChatContainer() {
  ...
  const [selectedProvider, setSelectedProvider] = React.useState('openai-mini');
  ...
  // Initialise dark mode from localStorage after mount (avoids SSR mismatch)
  React.useEffect(() => {
    const stored = localStorage.getItem('darkMode');
    const prefersDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    const dark = stored !== null ? stored === 'true' : prefersDark;
    setDarkMode(dark);
    setSelectedProvider(localStorage.getItem('selectedProvider') ?? 'openai-mini');
  }, []);
  ...
  React.useEffect(() => {
    axios.get<{ id: string; company: string; model: string }[]>(`${API_BASE}/api/providers`)
      .then(({ data }) => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errMessage: Message = { id: (Date.now() + 1).toString(), type: 'agent', content: 'Error: could not reach the agent. Is the client server running?', timestamp: new Date() };
        ...
        <div className={`${t.panelBg} border-b ${t.border} px-6 py-4 shadow-sm flex items-center justify-between`}>
          ...
          <div className="flex items-center gap-2">
            {availableProviders.length > 0 && (
              <select value={selectedProvider} onChange={e => { setSelectedProvider(e.target.value); localStorage.setItem('selectedProvider', e.target.value); }}
                ...
                {availableProviders.map(p => <option key={p.id} value={p.id}>{p.company} — {p.model}</option>)}
              </select>
            )}
            ...
            <div className="flex justify-start">
              <div className={`${t.cardBg} ${t.textPrimary} px-4 py-3 rounded-lg rounded-bl-none`}>
                <div className="flex space-x-2">
                  {[0, 0.1, 0.2].map(delay => <div key={delay} className={`w-2 h-2 ${t.mutedBg} rounded-full animate-bounce`} style={{ animationDelay: `${delay}s` }} />)}
                </div>
              </div>
            </div>
          )}
